...
This Resource Guide was created using the [`bookdown` package](https://github.com/rstudio/bookdown). `bookdown` provides a platform to publish references using various output formats (PDF, HTML, Word) and allows for interactivity, insertion of code chunks, and supports a variety of languages (R, Python, Julia, SQL, etc.). 
...
- *Note*: to compile to PDF, you need XeLaTeX. It is recommended to install TinyTeX (which includes XeLaTeX): <https://yihui.name/tinytex/>.
...
```{r bookdown, echo = F, auto_pdf = TRUE, fig.align = 'center', fig.height = 6, fig.alt="environment panel in RStudio with the build tab selected"}

knitr::include_graphics("images/build.png")
```


```{r gitselect, echo = F, auto_pdf = TRUE, fig.align = 'center', fig.height = 6, fig.alt="environment panel in RStudio with the build tab selected"}

knitr::include_graphics("images/select.png")
```

```{r gitcommit, echo = F, auto_pdf = TRUE, fig.align = 'center', fig.height = 6, fig.alt="environment panel in RStudio with the build tab selected"}
