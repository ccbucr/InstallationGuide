...
- navigate to the **Environment, Connections, Build, Git*\texttt{**\ panel,\ \ -\ select\ the\ **Build**\ tab,\ -\ select\ **Build\ Bookdown**\ -\ once\ the\ build\ is\ complete\ (the\ final\ line\ should\ read}Output created: docs/index/html`), commit, pull and push updates to GitHub via the \textbf{Git} tab.
